<?PHP
    ...
    $headTitle = 'Customer Feedback Report';
...
echo '<div class="row">';
	...
	echo '<div class="col-md-2">';
        ...
        echo '<select class="form-control" id="cmpl_fltID_rtpyeID" name="rtpyeID">';
		...
		echo '<option value="2" '.($rtpyeID == 2 ? 'selected="selected"' : '').'>All Customer Feedback Type</option>';
		echo '<option value="3" '.($rtpyeID == 3 ? 'selected="selected"' : '').'>All Complaints</option>';
		echo '<option value="4" '.($rtpyeID == 4 ? 'selected="selected"' : '').'>Completed Complaints</option>';
		echo '<option value="5" '.($rtpyeID == 5 ? 'selected="selected"' : '').'>Pending Complaints</option>';
		echo '<option value="6" '.($rtpyeID == 6 ? 'selected="selected"' : '').'>Commendation</option>';
		echo '<option value="7" '.($rtpyeID == 7 ? 'selected="selected"' : '').'>Customer Feedback Summary</option>';		
		echo '<option value="8" '.($rtpyeID == 8 ? 'selected="selected"' : '').'>Substained Complaints</option>';
		echo '<option value="9" '.($rtpyeID == 9 ? 'selected="selected"' : '').'>Unsubstained Complaints</option>';		
		echo '<option value="10" '.($rtpyeID == 10 ? 'selected="selected"' : '').'>Raw Data Report</option>';
        echo '</select>';
    echo '</div>';
	
    echo '<div class="col-xs-4" id="rpt_fields_divID" '.($rtpyeID == 10 ? '' : 'style="display:none;"').'>';
		...
		$Qry = $login->DB->prepare("SELECT * FROM rbuilder WHERE frmID In(4) Order By srID ASC ");								  
		if($Qry->execute())	
		{	
    ...
    echo '<div class="col-md-3">';
        ...
        $urlID = '?s=REPORT_COMMENTLINE&rtpyeID='.$rtpyeID.'&fromID='.$_GET['fromID'].'&toID='.$_GET['toID'].'&compID='.$companyID.'&rpt_fieldID='.implode(",",$rpt_fieldID);
        if($rtpyeID >= 1)
        {
...
?>    
				...
				<?PHP
                                    if(!empty($rtpyeID))
                                    { 
                                        if(($rtpyeID >= 1) && ($rtpyeID <= 9))
										{
											$reports->ReportDisplay($filters);
										}
										elseif(($rtpyeID >= 10) && ($rtpyeID <= 10))
										{
...
{	
	...
	mywindow.document.write('<html><head><title>Comment-Line Report</title>');
	mywindow.document.write('</head>');
	mywindow.document.write('<body><tr><th colspan="13" style="border:none 0px;"><b>Comment-Line Report</b></th></tr></br></br>');
...
$("#cmpl_fltID_rtpyeID").on('change',function()
{
	var rtpyeID = $("#cmpl_fltID_rtpyeID").val();
	if(parseInt(rtpyeID) == 10)
...
var urlID = 'rpt_n_cmline.php?rtpyeID='+rtpyeID+'&fromID='+fromID+'&toID='+toID;
 
$("#cmline_fltID_1,#cmline_fltID_2").on('change',function()	{setFilters(urlID);});
...
{
    var fltID_1 = $("#cmline_fltID_1").val();
    var fltID_2 = $("#cmline_fltID_2").val();
