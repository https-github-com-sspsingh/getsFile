<?PHP
    ...
    $headTitle = 'Personal Leave Report';
...
echo '<div class="row">';
    ...
    echo '<div class="col-md-2">';
        ...
        echo '<select class="form-control" id="sckl_fltID_rtpyeID" name="rtpyeID">';
        ...
        echo '<option value="2" '.($rtpyeID == 2 ? 'selected="selected"' : '').'>One Day Personal Leave</option>';
        echo '<option value="3" '.($rtpyeID == 3 ? 'selected="selected"' : '').'>Two Day Personal Leave</option>';
        echo '<option value="4" '.($rtpyeID == 4 ? 'selected="selected"' : '').'>Three Day Personal Leave</option>';
		//echo '<option value="6" '.($rtpyeID == 6 ? 'selected="selected"' : '').'>Summary of Sick Leave (Only Driver)</option>';
		//echo '<option value="7" '.($rtpyeID == 7 ? 'selected="selected"' : '').'>Summary of Sick Leave (Other Employees)</option>';
		echo '<option value="8" '.($rtpyeID == 8 ? 'selected="selected"' : '').'>Summary of Sick Leave</option>';
    ...
    echo '<div class="col-xs-4" id="rpt_fields_divID" '.($rtpyeID == 5 ? '' : 'style="display:none;"').'>';
		...
		echo '<select name="rpt_fieldID[]" id="rpt_fieldID" multiple="multiple" class="3col active">';
		$GET_rpt_fieldID = explode(",",(implode(",",$rpt_fieldID)));
		$Qry = $login->DB->prepare("SELECT * FROM rbuilder WHERE frmID In(2) Order By ID ASC ");								  
		if($Qry->execute())	
		{	
    ...
    echo '<div class="col-md-3">';
        ...
        if($rtpyeID >= 1)
        {
			$urlID = '?s=REPORT_SICKLEAVE&rtpyeID='.$rtpyeID.'&fromID='.$_GET['fromID'].'&toID='.$_GET['toID'].'&compID='.$companyID.'&rpt_fieldID='.implode(",",$rpt_fieldID);
			
            /*for($srID = 1; $srID <= 3; $srID++) 
            {
...
{	
	...
	mywindow.document.write('<html><head><title>Sick Leave Report</title>');
	mywindow.document.write('</head>');
	mywindow.document.write('<body><tr><th colspan="13" style="border:none 0px;"><b>Sick Leave Report</b></th></tr></br></br>');
...
$("#sckl_fltID_rtpyeID").on('change',function()
{
	var rtpyeID = $("#sckl_fltID_rtpyeID").val();
...
var urlID = 'rpt_n_sicklv.php?rtpyeID='+rtpyeID+'&fromID='+fromID+'&toID='+toID;
 
 
$("#sicklv_fltID_1,#sicklv_fltID_2,#sicklv_fltID_3").on('change',function()	{setFilters(urlID);});
...
{
    var fltID_1 = $("#sicklv_fltID_1").val();
    var fltID_2 = $("#sicklv_fltID_2").val();
    var fltID_3 = $("#sicklv_fltID_3").val();
    var fltID_7 = $("#FILTER_companyID").val();
    
    window.location = urlID + ('&fltID_1='+fltID_1+'&fltID_2='+fltID_2+'&fltID_3='+fltID_3+'&fltID_7='+fltID_7);
}
