...
echo '<div class="row">';
    ...
    echo '<div class="col-md-2">';
        ...
        echo '<select class="form-control" id="acc_fltID_rtpyeID" name="rtpyeID">';
        ...
        echo '<option value="2" '.($rtpyeID == 2 ? 'selected="selected"' : '').'>Accident Greater than $1000</option>';
        echo '<option value="3" '.($rtpyeID == 3 ? 'selected="selected"' : '').'>Responsible Greater than $1000</option>';
        echo '<option value="4" '.($rtpyeID == 4 ? 'selected="selected"' : '').'>Responsible Less than $1000</option>';
		echo '<option value="5" '.($rtpyeID == 5 ? 'selected="selected"' : '').'>Not Responsible Greater than $1000</option>';
        echo '<option value="6" '.($rtpyeID == 6 ? 'selected="selected"' : '').'>Not Responsible Less than $1000</option>';
		echo '<option value="7" '.($rtpyeID == 7 ? 'selected="selected"' : '').'>Raw Data Report</option>';
        echo '</select>';
    echo '</div>';
	
    echo '<div class="col-xs-4" id="rpt_fields_divID" '.($rtpyeID == 7 ? '' : 'style="display:none;"').'>';
		...
		$Qry = $login->DB->prepare("SELECT * FROM rbuilder WHERE frmID In(6) Order By ID ASC ");								  
		if($Qry->execute())	
		{	
	...
	echo '<div class="col-md-3">';
        ...
        $urlID = '?s=REPORT_ACCIDENT&rtpyeID='.$rtpyeID.'&fromID='.$_GET['fromID'].'&toID='.$_GET['toID'].'&compID='.$companyID.'&rpt_fieldID='.implode(",",$rpt_fieldID);
        if($rtpyeID >= 1)
        {
            /*for($srID = 1; $srID <= 6; $srID++) 
            {
...
{	
	...
	mywindow.document.write('<html><head><title>Accident Report</title>');
	mywindow.document.write('</head>');
	mywindow.document.write('<body><tr><th colspan="13" style="border:none 0px;"><b>  Accident Report</b></th></tr></br></br>');
...
$("#acc_fltID_rtpyeID").on('change',function()
{
	var rtpyeID = $("#acc_fltID_rtpyeID").val();
	if(parseInt(rtpyeID) == 7)
...
var urlID = 'rpt_n_accident.php?rtpyeID='+rtpyeID+'&fromID='+fromID+'&toID='+toID;
 
$("#accident_fltID_1,#accident_fltID_2,#accident_fltID_3,#accident_fltID_4,#accident_fltID_5,#accident_fltID_6").on('change',function()	{setFilters(urlID);});
...
{
    var fltID_1 = $("#accident_fltID_1").val();
    var fltID_2 = $("#accident_fltID_2").val();
    var fltID_3 = $("#accident_fltID_3").val();
    var fltID_4 = $("#accident_fltID_4").val();
    var fltID_5 = $("#accident_fltID_5").val();
    var fltID_6 = $("#accident_fltID_6").val();
    var fltID_7 = $("#FILTER_companyID").val();

    window.location = urlID + ('&fltID_1='+fltID_1+'&fltID_2='+fltID_2+'&fltID_3='+fltID_3+'&fltID_4='+fltID_4+'&fltID_5='+fltID_5+'&fltID_6='+fltID_6+'&fltID_7='+fltID_7);
}
