...
<?PHP
	...
	echo '<div class="col-xs-4" id="rpt_fields_divID" '.($rtpyeID == 2 ? '' : 'style="display:none;"').'>';
		...
		echo '<select name="rpt_fieldID[]" multiple="multiple" class="3col active">';
			...
			$Qry = $login->DB->prepare("SELECT * FROM rbuilder WHERE frmID In(11) Order By srID ASC ");								  
			if($Qry->execute())	
			{	
	...
	echo '<div class="col-md-3">';
        ...
        $urlID = '?s=REPORT_SIR&rtpyeID='.$rtpyeID.'&fromID='.$_GET['fromID'].'&toID='.$_GET['toID'].'&compID='.$companyID.'&rpt_fieldID='.implode(",",$rpt_fieldID);
        if($rtpyeID >= 1)
        {
