...
echo '<div class="row">';
    ...
    echo '<div class="col-md-2">';
        ...
        echo '<select class="form-control" id="inf_fltID_rtpyeID" name="rtpyeID">';
        ...
        echo '<option value="2" '.($rtpyeID == 2 ? 'selected="selected"' : '').'>Infringement Type</option>';
        echo '<option value="3" '.($rtpyeID == 3 ? 'selected="selected"' : '').'>Demerit Points</option>';
    ...
    echo '<div class="col-xs-4" id="rpt_fields_divID" '.($rtpyeID == 4 ? '' : 'style="display:none;"').'>';
		...
		$Qry = $login->DB->prepare("SELECT * FROM rbuilder WHERE frmID In(7) Order By ID ASC ");								  
		if($Qry->execute())	
		{	
	...
	echo '<div class="col-md-3">';
        ...
        $urlID = '?s=REPORT_INFRINGEMENT&rtpyeID='.$rtpyeID.'&fromID='.$_GET['fromID'].'&toID='.$_GET['toID'].'&compID='.$companyID.'&rpt_fieldID='.implode(",",$rpt_fieldID);
        if($rtpyeID >= 1)
        {
...
{	
	...
	mywindow.document.write('<html><head><title>Infringement Report</title>');
	mywindow.document.write('</head>');
	mywindow.document.write('<body><tr><th colspan="13" style="border:none 0px;"><b>Infringement Report</b></th></tr></br></br>');
...
$("#inf_fltID_rtpyeID").on('change',function()
{
	var rtpyeID = $("#inf_fltID_rtpyeID").val();
    ...
    var urlID = 'rpt_n_infrgs.php?rtpyeID='+rtpyeID+'&fromID='+fromID+'&toID='+toID;

    $("#infrgs_fltID_1,#infrgs_fltID_2").on('change',function()	{setFilters(urlID);});

    var setFilters = function(urlID)
    {
        var fltID_1 = $("#infrgs_fltID_1").val();
        var fltID_2 = $("#infrgs_fltID_2").val(); 
        var fltID_7 = $("#FILTER_companyID").val();
        
        window.location = urlID + ('&fltID_1='+fltID_1+'&fltID_2='+fltID_2+'&fltID_7='+fltID_7);
