...
<?PHP
    ...
    echo '<div class="col-md-2">';
		...
		//echo '<option value="1" '.($rtpyeID == 1  ? 'selected="selected"' : '').'>All ST Fare</option>';
		echo '<option value="2" '.($rtpyeID == 2 ? 'selected="selected"' : '').'>Raw Data Report</option>';
        echo '</select>';
    echo '</div>';
	
	echo '<div class="col-xs-4" id="rpt_fields_divID" '.($rtpyeID == 2 ? '' : 'style="display:none;"').'>';
		...
		echo '<select name="rpt_fieldID[]" multiple="multiple" class="3col active">';
			...
			$Qry = $login->DB->prepare("SELECT * FROM rbuilder WHERE frmID In(12) Order By srID ASC ");								  
			if($Qry->execute())	
			{	
	...
	echo '<div class="col-md-3">';
        ...
        $urlID = '?s=REPORT_STFARE&rtpyeID='.$rtpyeID.'&fromID='.$_GET['fromID'].'&toID='.$_GET['toID'].'&compID='.$companyID.'&rpt_fieldID='.implode(",",$rpt_fieldID);
        if($rtpyeID >= 1)
        {
...
{	
	...
	mywindow.document.write('<html><head><title>ST Fare Report</title>');
	mywindow.document.write('</head>');
	mywindow.document.write('<body><tr><th colspan="13" style="border:none 0px;"><b>ST Fare Report</b></th></tr></br></br>');
