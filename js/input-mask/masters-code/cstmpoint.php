	...
	public function view()
	{
		...
		if($Qry->execute())
		{
			...
			{ 
				...
				/* VOUCHER - DETAILS */
				$Qry = $this->DB->prepare("SELECT fileID_1 FROM cstpoint_dtl WHERE ID = :ID ORDER BY ID DESC ");
				...
				if($this->permissions['editID'] == 1 || $_SESSION[$this->website]['userTY'] == 'AD')
				{
					...
					if($uscountsID > 0)
					{
						echo '<td align="center"><a class="fa fa fa-users POPUP_uslogsID" aria-sort="'.($this->frmID.'_'.$row['ID'].'_Service Time Point\'s').'" 
