<?PHP
    ...
    $headTitle	=	'Incident Register';
    $titleText	=	$action ==	'view'	?	'All Incident Register List' : (!empty($id) ? 'Edit Incident' : 'Add New Incident' );
	
	/* SET - CRITERIA */
	$passSTR = "";
	$passSTR = ($dashID == 1 ? " " 
			  :($dashID == 2 ? " AND Date_Add(incident_regis.dateID, INTERVAL 7 DAY) < '".date("Y-m-d")."' " 
			  :($dashID == 3 ? " AND sincID = 1 " 
			  :($dashID == 4 ? " AND Date_Add(incident_regis.dateID, INTERVAL 7 DAY) < '".date("Y-m-d")."' AND sincID = 1 " : ""))));
...
<div class="row" style=" padding-left:10px !important;">
    ...
    <div class="col-md-1"><a href="<?=$login->home?>forms/<?=$basename?>" class="btn btn-primary btn-flat btn-flat fa fa-refresh">&nbsp;&nbsp;Clear Filters</a></div>
	
    <div class="col-md-1" style="margin-left:30px;">
        <!--<a onClick="exportToExcel()" class="btn btn-primary btn-flat btn-flat fa fa-download">&nbsp;&nbsp;Export Excel</a>-->
        
        <a class="btn btn-primary btn-flat btn-flat fa fa-download" href="<?=$login->home.'downloadCSV.php?s=INCIDENT_REGISTER&fdateID='.$fdateID.'&tdateID='.$tdateID.'&searchbyID='.$searchbyID.'&dashID='.$dashID.'&auditID='.$auditID?>"> Export Excel</a>
...
$(($("#fieldsID").val() == 1 ? "#increfno" : "#searchbyID")).focus();
