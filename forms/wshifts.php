<?PHP
	...
	$headTitle	=	'Weekly Roster';
	$titleText	=	$action ==	'view'	?	'All Weekly Roster List' : (!empty($id) ? 'Edit Weekly Roster' : 'Add New Weekly Roster' );
...
<!-- Right side column. Contains the navbar and content of the page -->
  ...
  <a href="<?=$alink?>"><button class="btn <?=$aclass?>"  style="margin-right:20px; float:right; display:inline-block" type="button">Add New</button></a>
                                ...
                                <a href="<?=$login->home.'forms/'.$basename.'?a='.$login->Encrypt('view')?>" class="btn btn-primary btn-flat" style="margin-right:50px; float:right">View List</a>
							...
							<?PHP
                                ...
                                {
									$masters->Form = 'Submit';
									$masters->allocateID($id);
                                    //$masters->Update_SatudaySunday($id);
								}
                            ?>
