<?PHP
    ...
    $headTitle	=   'Fare Evasion Register';
    $titleText	=   ($action ==	'view'	?	'All Fare Evasion Register List' : (!empty($id) ? 'Edit Fare Evasion Register' : 'Add New Fare Evasion Register' ));
...
<a class="btn btn-primary btn-flat btn-flat fa fa-download" href="<?=$login->home.'downloadCSV.php?s=STFARE_REGISTER&fdateID='.$fdateID.'&tdateID='.$tdateID.'&searchbyID='.$searchbyID.'&dashID='.$dashID.'&auditID='.$auditID?>"> Export Excel</a>
    </div>
</div>
</form>
<?PHP
							...
							<?PHP } ?>
                           	...
                           	<div class="box-body table-responsive"> 
								<?PHP 
									if($passSTR <> '')
									{
										$masters->filter_view($fdateID,$tdateID,$searchbyID,$passSTR);
									}
									else
									{
										if($action  == 'view' && $id <> '')	 $masters->createForm($id);
										elseif($action  == 'view')			 $masters->view($fdateID,$tdateID,$searchbyID,$auditID);
										elseif($action == 'create')			 $masters->createForm($id,$rowID);
									}
                                ?>
                            </div>
...
if(($("#fieldsID").val() == 1))
{
	...
	frmvalidator.addValidation("dayID","req","Enter Day ");
	frmvalidator.addValidation("shiftNO","req","Enter shift NO ");
	
	frmvalidator.addValidation("companyID","num","Select Depot Name");
	frmvalidator.addValidation("companyID","gt=0","Select Depot Name");
	
	frmvalidator.addValidation("routenoID","num","Select Route No");
	frmvalidator.addValidation("routenoID","gt=0","Select Route No");
	
	frmvalidator.addValidation("timeID","req","Enter Time");
	
	frmvalidator.addValidation("suburbID","num","Select Suburb");
	frmvalidator.addValidation("suburbID","gt=0","Select Suburb");
	
	frmvalidator.addValidation("routeInfo","req","Enter Route Info");
	
	frmvalidator.addValidation("descriptionID","num","Select Description");
	frmvalidator.addValidation("descriptionID","gt=0","Select Description");
	
	frmvalidator.addValidation("stopID","num","Select Route Location");
	frmvalidator.addValidation("stopID","gt=0","Select Route Location");
	
	frmvalidator.addValidation("nooffare","req","Enter Location"); 
