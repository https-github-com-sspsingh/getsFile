<?PHP
    ...
    $headTitle	= 'Manager Comments Register';
    $titleText	= $action ==	'view'	?	'All '.$headTitle.' List' : (!empty($id) ? 'Edit '.$headTitle : 'Add New '.$headTitle);
...
<div class="row" style=" padding-left:10px !important;">
	...
	<div class="col-md-1" style="margin-left:30px;">
        <a class="btn btn-primary btn-flat btn-flat fa fa-download" href="<?=$login->home.'downloadCSV.php?s=MNGCOMMENTS_REGISTER&fdateID='.$fdateID.'&tdateID='.$tdateID.'&searchbyID='.$searchbyID.'&dashID='.$dashID.'&auditID='.$auditID?>"> Export Excel</a>
    </div>
</div>
</form>
<?PHP
...
if(($("#fieldsID").val() == 1))
{
    ...
    frmvalidator.addValidation("staffID","num","Plz select Driver Name");
    frmvalidator.addValidation("staffID","gt=0","Plz select Driver Name ");

    frmvalidator.addValidation("invID","num","Plz select Interviewed By");
    frmvalidator.addValidation("invID","gt=0","Plz select Interviewed By");

	frmvalidator.addValidation("wrtypeID","num","Plz select Warning Type");
    frmvalidator.addValidation("wrtypeID","gt=0","Plz select Warning Type");
