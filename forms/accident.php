<?PHP
    ...
    $headTitle	=   'Accidents Register';
    $titleText	=   $action ==	'view'	? 'All Accidents Register List' : (!empty($id) ? 'Edit Accidents' : 'Add New Accidents' );
    $titleText  =   ($action == 'popups' ? 'Record of Action ' : $titleText);
	
	/* SET - CRITERIA */
	$passSTR = "";
	$passSTR = ($dashID == 1 ? " AND Date_Add(accident_regis.dateID, INTERVAL 7 DAY) <= '".date("Y-m-d")."' AND (engdoneID <= 0 || oprdoneID <= 0) " 
			  :($dashID == 2 ? " AND DATEDIFF(CURDATE(),dateID) <= 7 AND (engdoneID <= 0 || oprdoneID <= 0) " 
...
<!-- Right side column. Contains the navbar and content of the page -->
            <aside class="right-side">      
                ...
                <?PHP 
                    if($action == 'view' || ($action == 'create' && !empty($id)) ) 
                    {
                        ...
                        {
                            $aclass="btn-primary btn-flat btn-flat"; $alink	= $basename.'?a='.$masters->Encrypt("create");
                        }
                        else
                        {
  ...
  <a href="<?=$alink?>"><button class="btn <?=$aclass?>"  style="margin-right:20px; float:right; display:inline-block" type="button">Add New</button></a>
								...
								<?PHP }  if($action <> 'view') { ?> 
                                    ...
                                    <button class="btn btn-primary btn-flat btn-flat" 
...
<div class="row" style=" padding-left:10px !important;">
    ...
    <div class="col-md-1">
    	<input type="submit" name="Submit" class="btn btn-primary btn-flat btn-flat btn-flat fa fa-filter" value="&nbsp;Filter Data" />
    </div>
    
    <div class="col-md-1">
    	<a href="<?=$login->home?>forms/<?=$basename?>" class="btn btn-primary btn-flat btn-flat btn-flat fa fa-refresh">&nbsp;&nbsp;Clear Filters</a>
    </div>
	
    <div class="col-md-1" style="margin-left:30px;">
		<a class="btn btn-primary btn-flat btn-flat btn-flat fa fa-download" href="<?=$login->home.'downloadCSV.php?s=ACCIDENT_REGISTER&fdateID='.$fdateID.'&tdateID='.$tdateID.'&searchbyID='.$searchbyID.'&dashID='.$dashID.'&auditID='.$auditID?>"> Export Excel</a>
    </div>
...
$(($("#fieldsID").val() == 1 ? "#accrefno" : "#searchbyID")).focus();  
