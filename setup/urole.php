<?PHP
	...
	$headTitle	=	'User Role\'s';
	$titleText	=	$action ==	'view'	?	'All User Role\'s List' : (!empty($id) ? 'Edit User Role\'s' : 'Add New User Role\'s' );
	
?>
<input type="hidden" id="fieldID" value="100" />
<input type="hidden" id="fieldsID" value="<?=($action == 'view' ? 0 : 1)?>" />
<!-- Right side column. Contains the navbar and content of the page -->
            <aside class="right-side <?=($action == 'create' ? 'strech' : '')?>">      
...
if(($("#fieldsID").val() == 1))
{
	...
	frmvalidator.addValidation("title","req","Enter Form User Role's Name ");
}
</script>