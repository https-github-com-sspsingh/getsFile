<?PHP
    ...
    $headTitle	= 'Company Master';
    $titleText	= ($action ==	'view'	? 'All Company Master List' : (!empty($id) ? 'Edit Company' : 'Add New Company'));
...
<!-- Right side column. Contains the navbar and content of the page -->
  ...
  <a href="<?=$alink?>"><button class="btn <?=$aclass?>"  style="margin-right:20px; float:right; display:inline-block" type="button">Add New</button></a>
									...
									<?PHP }  if($action <> 'view') { ?> 
										<button class="btn btn-primary btn-flat" onclick="createURL('<?=$basename?>');" style="margin-right:50px; float:right" type="button" >View List</button>
									<?PHP	}	?>
...
if(($("#fieldsID").val() == 1))
{
	...
	frmvalidator.addValidation("title","req","Enter Company Name ");
    frmvalidator.addValidation("address","req","Enter Address ");
    frmvalidator.addValidation("pscode","req","Enter P S Code ");
	frmvalidator.addValidation("code","req","Enter Depot Code ");
	
	frmvalidator.addValidation("dcdaysID","req","Enter Days");
	
	frmvalidator.addValidation("dsnotID","num","Select Dashboard Slips");
	frmvalidator.addValidation("dsnotID","gt=0","Select Dashboard Slips");
}
</script>