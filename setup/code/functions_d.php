...
class DFunctions extends Functions
{
    ...
    public function SignOn_Listsing_1($arrDATA)
    {
        ...
        if($Qry->execute())
        {
            ...
            if(is_array($this->rows_1) && count($this->rows_1) > 0)
            {
                ...
                foreach($this->rows_1 as $rows_1)
                {
                    $totD   = $this->count_rows('imp_shift_daily'," LEFT JOIN shift_masters_dtl ON shift_masters_dtl.ID = imp_shift_daily.shiftID AND shift_masters_dtl.recID = imp_shift_daily.shift_recID WHERE If(shift_masters_dtl.fID_019 = 'N' AND imp_shift_daily.tagCD = 'B', 0, 1) = 1 AND imp_shift_daily.singinID <> '' AND imp_shift_daily.companyID = ".$rows_1['ID']." AND If(imp_shift_daily.fID_018 > 0, imp_shift_daily.fID_018, imp_shift_daily.fID_013) > 0 AND imp_shift_daily.choppedID <= 0 AND imp_shift_daily.singinFR = 'TOUCHPAD' AND (DATE(imp_shift_daily.dateID) BETWEEN '".$this->dateFormat($fdateID)."' AND '".$this->dateFormat($tdateID)."') ");
					
					$SQL = "Select Sum(FO.countID) As countID From (Select Profile_2.empID, Count(1) As countID, Sec_To_Time(Sum(Time_To_Sec(Profile_2.diffTm))) As timeDF From (Select imp_shift_daily.recID, If(imp_shift_daily.fID_018 > 0, imp_shift_daily.fID_018, imp_shift_daily.fID_013) As empID,
    ...
    public function SignOn_Listsing_2($arrDATA)
    {
		...
		imp_shift_daily.choppedID <= 0 And imp_shift_daily.companyID In(".$companyID.") And imp_shift_daily.dateID BETWEEN '".$this->dateFormat($fdateID)."' AND '".$this->dateFormat($tdateID)."' And If(shift_masters_dtl.fID_019 = 'N' And imp_shift_daily.tagCD = 'B', 0, 1) = 1 And TimeDiff((Time(If(imp_shift_daily.tagCD = 'A', shift_masters_dtl.fID_2, If(imp_shift_daily.tagCD = 'B', shift_masters_dtl.fID_9, '')))), (Time(imp_shift_daily.singinID))) Like '%-%' And
    ...
    public function SignOn_Listsing_3($arrDATA)
    {
		...
		If(imp_shift_daily.fID_018 > 0, imp_shift_daily.fID_018, imp_shift_daily.fID_013) = ".$employeeID." ".$sortFILTER;
	...
	public function SignOn_Listsing_4($arrDATA)
    {
		...
		From imp_shift_daily Inner Join shift_masters_dtl On shift_masters_dtl.recID = imp_shift_daily.shift_recID Where imp_shift_daily.choppedID <= 0 And imp_shift_daily.companyID In (".$companyID.") And imp_shift_daily.dateID BETWEEN '".$this->dateFormat($fdateID)."' AND '".$this->dateFormat($tdateID)."' And imp_shift_daily.singinFR = 'TOUCHPAD' AND 
		If(shift_masters_dtl.fID_019 = 'N' And imp_shift_daily.tagCD = 'B', 0, 1) = 1 And TimeDiff((Time(If(imp_shift_daily.tagCD = 'A', shift_masters_dtl.fID_2, If(imp_shift_daily.tagCD = 'B', shift_masters_dtl.fID_9, '')))), (Time(imp_shift_daily.singinID))) Like '%-%' ".$crtID." ".$sortFILTER;
