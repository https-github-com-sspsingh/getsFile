...
class Masters extends SFunctions
{
    ...
    public function view()
    {
        ...
        {    
            ...
            if($Qry->execute())
            {
                ...
                echo '<th>Field Criteria</th>';
				echo '<th>Field Criteria - 2</th>';
                ...
                foreach($this->rows as $row)			
                {
                    ...
                    echo '<td>'.($row['ftypeID'] == 1 ? 'Text-Box' 
                               ...
                               :($row['ftypeID'] == 5 ? 'Text-Area' : ''))))).'</td>';
                    echo '<td>'.$row['filedCR'].'</td>';
                    echo '<td>'.($row['tcaseID'] > 0 ? '= "" ' : '').'</td>';
    ...
    echo '<div class="row">';		
        echo '<div class="col-xs-4">';
            ...
            echo '<option aria-title="incident" aria-sort="incident_regis" value="5" '.($frmID == 5 ? 'selected="selected"' : '').'>Incident - Security No</option>';
            ...
            echo '<option aria-title="incident" aria-sort="incident_regis" value="10" '.($frmID == 10 ? 'selected="selected"' : '').'>Incident - Security Yes</option>';
            echo '</select>';
    ...
    echo '<div class="row">';		
        ...
        echo '<div class="col-xs-2">';
                echo '<label for="section">Show/Hide <span class="Maindaitory">*</span></label>';
                echo '<select name="visibleID" id="visibleID" class="form-control" >';
                echo '<option value="0" selected="selected">-- Select Visible --</option>';
                    $visibleID = (!empty($id) ? ($this->result['visibleID']) : 1);
                    echo '<option value="1" '.($visibleID == 1 ? 'selected="selected"' : '').'>Show</option>';
                    echo '<option value="2" '.($visibleID == 2 ? 'selected="selected"' : '').'>Hide</option>';
                echo '</select>';
            echo '<span id="register_visibleID_errorloc" class="errors"></span>';
        echo '</div>';
        
        echo '<div class="col-xs-2">';
                echo '<label for="section">AND/Or <span class="Maindaitory">*</span></label>';
                echo '<select name="paramID" id="paramID" class="form-control" >';
                echo '<option value="0" selected="selected">-- Select --</option>';
                    $paramID = (!empty($id) ? ($this->result['paramID']) : 1);
                    echo '<option value="1" '.($paramID == 1 ? 'selected="selected"' : '').'>Or</option>';
                    echo '<option value="2" '.($paramID == 2 ? 'selected="selected"' : '').'>AND</option>';
                echo '</select>';
				echo '<span id="register_paramID_errorloc" class="errors"></span>';
        echo '</div>';
    echo '</div><br />';
    ...
    echo '<div class="row">';		
        ...
        echo '<div class="col-xs-3">';
                echo '<label for="section">Criteria <span class="Maindaitory">*</span></label>';
                echo '<select name="caseID" id="caseID" class="form-control" >';
                $caseID = !empty($id) ? ($this->result['caseID']) : $this->safeDisplay('caseID');
                echo '<option value="0" selected="selected">-- Select Criteria --</option>';
                    echo '<option value="1" '.($caseID == 1 ? 'selected="selected"' : '').'>IS NULL</option>';
                    echo '<option value="2" '.($caseID == 2 ? 'selected="selected"' : '').'><> 0</option>';
                    echo '<option value="3" '.($caseID == 3 ? 'selected="selected"' : '').'>= 0</option>';
                    echo '<option value="4" '.($caseID == 4 ? 'selected="selected"' : '').'>>= 0</option>';
                    echo '<option value="5" '.($caseID == 5 ? 'selected="selected"' : '').'>> 0</option>';
                    echo '<option value="6" '.($caseID == 6 ? 'selected="selected"' : '').'><= 0</option>';
                    echo '<option value="7" '.($caseID == 7 ? 'selected="selected"' : '').'>< 0</option>';
                    echo '<option value="8" '.($caseID == 8 ? 'selected="selected"' : '').'>= 1</option>';
                echo '</select>';
				echo '<span id="register_caseID_errorloc" class="errors"></span>';
        echo '</div>';
		
        echo '<div class="col-xs-3">';
                echo '<label for="section">Criteria - 2 </label>';
                echo '<select name="tcaseID" id="tcaseID" class="form-control" >';
                $tcaseID = !empty($id) ? ($this->result['tcaseID']) : $this->safeDisplay('tcaseID');
					echo '<option value="0" selected="selected">-- Select Criteria --</option>';
					echo '<option value="1" '.($tcaseID == 1 ? 'selected="selected"' : '').'>= ""</option>';
                echo '</select>';
        echo '</div>';
    echo '</div>';
    ...
    {	
        if($this->Form_Variables() == true)
        {
            ...
            {
                $_POST['filedCR'] = ($_POST['caseID'] == 1 ? 'IS NULL' 
                                   :($_POST['caseID'] == 2 ? '<> 0' 
                                   :($_POST['caseID'] == 3 ? '= 0' 
                                   :($_POST['caseID'] == 4 ? '>= 0' 
                                   :($_POST['caseID'] == 5 ? '> 0' 
