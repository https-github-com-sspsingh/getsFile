<?PHP
	...
	$headTitle	=	'Service Time Point\'s';
	$titleText	=	$action ==	'view'	?	'All Service Time Point\'s List' : (!empty($id) ? 'Edit Service Time Point\'s' 
	: 'Add New Service Time Point\'s' );
...
<!-- Right side column. Contains the navbar and content of the page -->
  ...
  <a href="<?=$alink?>"><button class="btn <?=$aclass?>"  style="margin-right:20px; float:right; display:inline-block" type="button">Add New</button></a>
								...
								<?PHP }  if($action <> 'view') { ?> 
                                    ...
                                    <a class="btn btn-primary btn-flat" href="<?=$masters->home.'masters/'.$basename?>?a=<?=$masters->Encrypt('view')?>" style="margin-right:50px; float:right">View All Lists</a>
                                <?PHP	}	?>
...
if(($("#fieldsID").val() == 1))
{
	...
	frmvalidator.addValidation("contractID","num","Plz select Contracte");
    frmvalidator.addValidation("contractID","gt=0","Plz select Contracte ");
	
	frmvalidator.addValidation("serviceID","num","Plz select Service No");
    frmvalidator.addValidation("serviceID","gt=0","Plz select Service No ");
