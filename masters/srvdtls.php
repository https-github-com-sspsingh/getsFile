<?PHP
	...
	$masters = new Masters();
        ...
        $titleText  =	$action ==	'view'	?	'All Service Details List' : (!empty($id) ? 'Edit Service Details' : 'Add New Service Details' );
...
<!-- Right side column. Contains the navbar and content of the page -->
  ...
  <a href="<?=$alink?>"><button class="btn <?=$aclass?>"  style="margin-right:20px; float:right; display:inline-block" type="button">Add New</button></a>
								...
								<?PHP }  if($action <> 'view') { ?> 
                                    ...
                                    <a class="btn btn-primary btn-flat" href="<?=$masters->home?>masters/srvdtls.php?a=<?=$masters->Encrypt('view')?>" style="margin-right:50px; float:right">View All Lists</a>
                                <?PHP	}	?>
...
if(($("#fieldsID").val() == 1))
{
	...
	frmvalidator.addValidation("codeID","req","Enter E. Code ");
	
	frmvalidator.addValidation("title","req","Enter P.Decsription ");
	frmvalidator.addValidation("locationID","req","Enter Departure ");
	frmvalidator.addValidation("locationID1","req","Enter Destination ");
}
</script>