<?PHP
	...
	$headTitle	=	'Bus Details';
	$titleText	=	$action ==	'view'	?	'All Bus Details List' : (!empty($id) ? 'Edit Bus Details' : 'Add New Bus Details' );
...
<!-- Right side column. Contains the navbar and content of the page -->
  ...
  <a href="<?=$alink?>"><button class="btn <?=$aclass?>"  style="margin-right:20px; float:right; display:inline-block" type="button">Add New</button></a>
								...
								<?PHP }  if($action <> 'view') { ?> 
                                    ...
                                    <a class="btn btn-primary btn-flat" href="<?=$masters->home?>masters/buses.php?a=<?=$masters->Encrypt('view')?>" style="margin-right:50px; float:right">View All Lists</a>
                                <?PHP	}	?>
...
if(($("#fieldsID").val() == 1))
{
	...
	frmvalidator.addValidation("busno","req","Enter Bus No. ");
	frmvalidator.addValidation("lcsno","req","Enter Licence No ");
	frmvalidator.addValidation("depot","req","Enter Depot Name ");
	frmvalidator.addValidation("nos","req","Enter Seats ");
}
</script>