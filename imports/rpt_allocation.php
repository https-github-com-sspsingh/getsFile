<?PHP
    ...
    include '../header.php';
    include '../sidebar.php';
    include 'code/rpt_allocation.php';
    $reports = new Reports();
...
<div class="row">
    ...
    <div class="col-md-4">
        ...
        {	
            ?>
            <label for="section">&nbsp;</label>
            <a href="<?=$login->home.'exportCSV.php?s=ALLOCATION_SHEET&fromID='.$_POST['fromID']?>" class="btn btn-danger btn-flat fa fa-download">&nbsp;&nbsp;Export Excel</a>
            <?PHP
        }
		
        if($login->GET_menusPerms('97') == 1 && $_POST['fromID'] <> '')
        {	
            ?>
            <label for="section">&nbsp;</label>
            <a class="btn bg-maroon btn-flat margin fa fa-save" style="margin-top:-1px;" target="new" href="<?=$login->home?>imports/newshifts.php?a=<?=$login->Encrypt('create')?>&date=<?=$login->Encrypt($_POST['fromID'])?>&companyID=<?=$login->Encrypt($_SESSION[$login->website]['compID'])?>">&nbsp;&nbsp;ADD NEW SHIFTS</a>
			<?PHP
        }
        ...
        {	
            ?>
            <label for="section">&nbsp;</label>
            <a style="margin-top:-1px;" aria-sort="<?=$_POST['fromID']?>" class="btn bg-maroon btn-flat margin fa fa-save DisplayPendingShifts">&nbsp;&nbsp;SEE - PENDING SHIFTS</a>
            <?PHP
        }
        ?>
    </div> 
    ...
    <?PHP } ?>
				<div class="box-body table-responsive">
					<?PHP	if(!empty($rtpyeID))	{$reports->ReportDisplay($filters);}	?>
				</div>
				</div><!-- /.box -->
				</div>
				</section><!-- /.content -->
	...
	$(".update-signon-cuttoff").on('click',function()
	{
		if($(this).is(':checked'))  
		{
			$.ajax({
				...
				{
					if(data.success == true)	{alert('Shift Status Is Updated...');}
				},
...
function PrintPage(elem)
{
    ...
    {
        if(parseInt(recID) > 0 && fieldNM != '' && valueTEXT != '')
        {		
            $.ajax({			
            url : '../ajax/ajax_DBpopups.php',
            type:'POST',
