...
class Imports extends SFunctions
{
    ...
    public function GoToInnserSheet($Status = 1)
    {
        ...
        if(!empty($sheetID) && ($optionID == 1) && ($chooseID == 1) && !empty($day_ID))
        {
				  ...
				  if($setID == 2)
				  {
						...
						for($i = 3; $i <= $arrayCount; $i++)
						{
							...
							if(!empty($dateID) && (!empty($tagCD_A) || !empty($tagCD_B)) && (empty($checkID) || $checkID == 0) && ($resultID > 0))
							{
								...
								if($tagCD_A == 'A')		/* INSERT - TAG - A - CODES */
								{
									$arrA = array();
								...
								if($tagCD_B == 'B')		/* INSERT - TAG - B - CODES */
								{
									$arrB = array();
									...
									/*
										...
										{
											if($coloumVALUE == 'N'|| $coloumVALUE == 'n')
												$array['cuttoffID'] = 1;
											else
												$array['cuttoffID'] = 0;
										}
											...
											$array['cuttoffID'] = 0;
										else if($day_ID == 2 && $tickID == 0)
											$array['cuttoffID'] = 1;
										else
											$array['cuttoffID'] = 0;
									*/
									//$arrB['cuttoffID'] = (($dayID == 'Saturday' || $dayID  == 'Sunday') ? (($coloumVALUE == 'N'|| $coloumVALUE == 'n') ? 1 : 0) :($day_ID == 1 ? 0 :($day_ID == 2 && $tickID == 0 ? 1 : 0)));									
									//$arrB['cuttoffID'] = (($dayID == 'Saturday' || $dayID  == 'Sunday') ? 1 :($day_ID == 1 ? (($coloumVALUE == 'N' || $coloumVALUE == 'n') ? 1 : 0) :(($day_ID == 2 && $tickID == 0) ? 1 : 0)));									
				...
				if(is_array($this->rows_1) && count($this->rows_1) > 0)			
				{
					...
					foreach($this->rows_1 as $rows_1)
					{
						...
						echo '<tr>';
								...
								echo '<td align="center">'.strtoupper($busNAME).'</td>';
